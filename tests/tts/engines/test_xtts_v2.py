"""Tests for the XTTS v2 (Coqui) cloning engine and its registration."""
...
def test_xtts_v2_is_registered() -> None:
    assert "xtts_v2" in TTS_ENGINES.available()


def test_xtts_v2_declares_cloning_support() -> None:
    engine = TTS_ENGINES.create("xtts_v2")
    assert engine.name == "xtts_v2"
    assert engine.supports_cloning is True


def test_xtts_v2_rejects_preset_only_voice() -> None:
...
def test_xtts_v2_rejects_empty_text() -> None:
    engine = TTS_ENGINES.create("xtts_v2")
...
@pytest.mark.slow
def test_xtts_v2_real_synthesis_produces_16k_mono_audio() -> None:
    """Real-weights smoke test: needs the coqui-tts package, torch, and a GPU
