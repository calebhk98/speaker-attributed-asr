"""Tests for the Chatterbox engine: fast contract checks plus a slow
...
from satasr.tts.engines.chatterbox import ChatterboxTTSEngine
...
def test_chatterbox_is_registered() -> None:
    assert "chatterbox" in TTS_ENGINES.available()


def test_chatterbox_metadata() -> None:
    engine = TTS_ENGINES.create("chatterbox")
    assert engine.name == "chatterbox"
    assert engine.supports_cloning is True


def test_chatterbox_rejects_preset_only_voice() -> None:
...
def test_chatterbox_rejects_empty_text() -> None:
    engine = ChatterboxTTSEngine()
...
@pytest.mark.slow
def test_chatterbox_real_synthesis_produces_16k_mono_audio() -> None:
