"""Tests for the GPT-SoVITS cloning engine and its registration."""
...
def test_gpt_sovits_is_registered() -> None:
    assert "gpt_sovits" in TTS_ENGINES.available()


def test_gpt_sovits_declares_cloning_support() -> None:
    engine = TTS_ENGINES.create("gpt_sovits")
    assert engine.name == "gpt_sovits"
    assert engine.supports_cloning is True


def test_gpt_sovits_rejects_preset_only_voice() -> None:
...
def test_gpt_sovits_rejects_empty_text() -> None:
    engine = TTS_ENGINES.create("gpt_sovits")
...
@pytest.mark.slow
def test_gpt_sovits_real_synthesis_produces_16k_mono_audio() -> None:
    """Real-weights smoke test: needs the GPT_SoVITS package, pretrained GPT
    and SoVITS checkpoints, and a GPU (or a slow CPU fallback) that are not
