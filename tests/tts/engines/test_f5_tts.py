"""Tests for the F5-TTS cloning engine and its registration."""
...
def test_f5_tts_is_registered() -> None:
    assert "f5_tts" in TTS_ENGINES.available()


def test_f5_tts_declares_cloning_support() -> None:
    engine = TTS_ENGINES.create("f5_tts")
    assert engine.name == "f5_tts"
    assert engine.supports_cloning is True


def test_f5_tts_rejects_preset_only_voice() -> None:
...
def test_f5_tts_rejects_empty_text() -> None:
    engine = TTS_ENGINES.create("f5_tts")
...
@pytest.mark.slow
def test_f5_tts_real_synthesis_produces_16k_mono_audio() -> None:
    """Real-weights smoke test: needs the f5-tts package, torch, and a GPU
    (or a slow CPU/diffusion fallback) that are not available in this
