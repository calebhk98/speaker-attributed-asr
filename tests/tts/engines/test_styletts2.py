"""Tests for the StyleTTS 2 cloning engine and its registration."""
...
def test_styletts2_is_registered() -> None:
    assert "styletts2" in TTS_ENGINES.available()


def test_styletts2_declares_cloning_support() -> None:
    engine = TTS_ENGINES.create("styletts2")
    assert engine.name == "styletts2"
    assert engine.supports_cloning is True


def test_styletts2_rejects_preset_only_voice() -> None:
...
def test_styletts2_rejects_empty_text() -> None:
    engine = TTS_ENGINES.create("styletts2")
...
@pytest.mark.slow
def test_styletts2_real_synthesis_produces_16k_mono_audio() -> None:
    """Real-weights smoke test: needs the styletts2 package, torch, and a GPU
